...
class SDE_Detector(Detector):
    ...
    def postprocess(self,
        ...
        car_ids = [2,5,7]
        ids = []
        for i, cid in enumerate(pred_cls_ids):
            if cid in car_ids:
                ids.append(i)
        ids = np.array(ids)

        pred_xyxys = pred_xyxys[ids]
        pred_scores = pred_scores[ids]
        pred_cls_ids = pred_cls_ids[ids]
...
def predict_video(detector, reid_model, camera_id):
    ...
    while (1):
        ...
        car_detect_time.append(time.time() - stime)
        if len(pred_dets) == 1 and np.sum(pred_dets) == 0:
            print('Frame {} has no object, try to modify score threshold.'.
                  format(frame_id))
            timer.toc()
            ...
            timer.toc()
            reid_time.append(time.time() - stime)
            total_time.append(timer.duration)
    ...
    # else:
    #     writer.release()
    print(sum(car_detect_time) / len(car_detect_time))
    print(sum(reid_time) / len(reid_time))
